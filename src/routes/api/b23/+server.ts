    ...
    try {
        ...
        if (response.status === 301 || response.status === 302) {
            ...
            if (locationHeader) {
                ...
                console.error(`b23.tv redirect for ${targetUrl} missing Location header. Status: ${response.status}`);
                throw error(500, 'b23.tv redirect response missing Location header.');
            }
        } else {
            const responseText = await response.text().catch(() => "Could not read response body");
            console.warn(`b23.tv did not redirect for ${targetUrl}. Status: ${response.status}. Body: ${responseText}`);
            throw error(response.status, `b23.tv did not redirect as expected. Status: ${response.status}`);
        }
    } catch (e: any) {
        if (e.status && e.body) {
            throw e;
