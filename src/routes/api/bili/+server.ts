...
export const GET: RequestHandler = async ({ url }) => {
  ...
  if (!ALLOWED_API_HOSTS.includes(parsedProxyUrl.hostname)) {
    throw error(403, `Host not allowed: ${parsedProxyUrl.hostname}. Only Bilibili APIs are permitted.`);
  }
  ...
  requestHeaders.append('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'); // A common user agent
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw error(response.status, `Bilibili API Error (${response.status}): ${response.statusText}. Response: ${errorBody}`);
    }
